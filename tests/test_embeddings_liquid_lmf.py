...
def test_init_passes_revision_and_device_to_sentence_transformer(fake_st) -> None:
    ...
    LiquidLMFEmbedder(
        model="LiquidAI/LFM2.5-Embedding-350M", revision="f35ae2c91d687658dbf1f2b449382f0b019b9808"
    )
...
def test_init_requires_model_and_revision(fake_st) -> None:
    """No class-level defaults: caller must pass model and revision explicitly.

    The singleton factory in infrastructure/embedder.py is the only
    production caller and threads from Settings. Class-level defaults
    were an unused YAGNI surface (see audit-architecture-2026-07-19
    finding 2.1).
    """
    from lean.embeddings.liquid_lmf import LiquidLMFEmbedder
