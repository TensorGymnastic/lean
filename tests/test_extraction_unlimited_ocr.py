"""Tests for Unlimited-OCR HTTP client (vLLM)."""
...
def test_extract_markdown_success(tmp_path: Path) -> None:
    """Successful vLLM call returns markdown."""
    ...
    markdown, page_count = extract_markdown(pdf_path, vllm_base_url="http://fake-vllm:8000")
...
def test_extract_markdown_raises_on_503(tmp_path: Path) -> None:
    ...
    respx.post("http://fake-vllm:8000/v1/chat/completions").mock(return_value=httpx.Response(503))
    pdf_path = tmp_path / "test.pdf"
    _write_minimal_pdf(pdf_path)
...
def test_extract_markdown_raises_on_connection_error(tmp_path: Path) -> None:
    """Connection error (vLLM not running) raises OCRBackendUnavailable."""
    pdf_path = tmp_path / "test.pdf"
    _write_minimal_pdf(pdf_path)
    ...
    with pytest.raises(OCRBackendUnavailable):
        ...
        extract_markdown(
            pdf_path,
            vllm_base_url="http://127.0.0.1:59999",
            timeout=2.0,
        )
