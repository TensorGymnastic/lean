...
class _SearchPatcher:
    ...
    def __enter__(self):
        ...
        s(patch("lean.services.search.get_settings", return_value=self.settings))
        s(patch("lean.services.search.get_embedder", return_value=self.mock_embedder))
        s(patch("lean.services.search.get_llm", return_value=None))

        mock_store_cls = MagicMock()
        mock_store_cls.from_env.return_value = self.mock_conn
        s(patch("lean.services.search.StoreConnection", mock_store_cls))
        ...
        s(patch("lean.services.search.AnalyticsRepo", mock_analytics_cls))
...
class TestSearchBasic:
    ...
    def test_returns_chunks_from_vector_search(self, valid_env):
        ...
        with _SearchPatcher(settings, vector_hits=vector_hits):
            results = search_mod.search("What is DMAIC?")
    ...
    def test_empty_results_when_no_hits(self, valid_env):
        ...
        with _SearchPatcher(settings, vector_hits=[]):
            results = search_mod.search("nonexistent")
    ...
    def test_embed_query_called_with_search_query(self, valid_env):
        ...
        with _SearchPatcher(settings, vector_hits=[_make_hit(1, 0.5)]) as p:
            search_mod.search("my query text")
...
class TestSearchTopK:
    ...
    def test_truncates_to_k(self, valid_env):
        ...
        with _SearchPatcher(settings, vector_hits=vector_hits):
            results = search_mod.search("query", k=3)
