...
async def ingest_url(url: str) -> IngestResult:
    ...
    return await _ingest_pdf(url, pipeline=get_pipeline())
...
async def ingest_url_rest(payload: dict[str, object]) -> dict[str, object]:
    ...
    result = await _ingest_pdf(url, pipeline=get_pipeline())
...
def ingest_url_cli(url: str, json_output: bool = typer.Option(False, "--json")) -> None:
    """Fetch a URL and ingest its content."""
    result = asyncio.run(_ingest_pdf(url, pipeline=get_pipeline()))
...
def ingest_list_cli(
    ...
    for url in urls:
        try:
            res = asyncio.run(_ingest_pdf(url, pipeline=get_pipeline()))
            success += 1
